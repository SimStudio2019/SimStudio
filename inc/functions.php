<?php
	...
	function breadcrumb($url){
		...
		foreach($paths as $path){
			$request = 'SELECT url, title
			FROM page
			WHERE url LIKE :url
			LIMIT 0,1';
			$prepare=$bdd->prepare($request);
			$prepare->bindValue(':url', '%'.$path, PDO::PARAM_STR);
			$prepare->execute();
			$results = $prepare->fetchAll(PDO::FETCH_ASSOC);
			...
			if(!isset($results[0])) 
				...
				$links[$results[0]['title']] = $results[0]['url'];
		...
		if(!isset($error_path) && !in_array($last, $page_without_breadcrumb))
			return $breadcrumb;
		else
			return false;
	}